    ...
    <AuthLayout title="Welcome back" subtitle="Sign in to continue building">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="liquid-glass rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <button type="button" onClick={() => setShowPassword(!showPassword)} aria-label="Toggle password visibility" className="cursor-pointer">{showPassword ? <EyeOff className="w-4 h-4 text-white/60" /> : <Eye className="w-4 h-4 text-white/60" />}</button>
        </div>
        <div className="flex justify-end"><Link to="/forgot-password" className="text-xs text-white/50 hover:text-white/80">Forgot password?</Link></div>
        <button type="submit" disabled={isLoading} className="liquid-glass-strong rounded-full py-3 h-11 w-full mt-4 text-white font-medium text-sm hover:scale-105 disabled:hover:scale-100 transition-all outline-none flex justify-center items-center cursor-pointer disabled:cursor-not-allowed">{isLoading ? <LoadingSpinner /> : "Sign In"}</button>
      </form>
