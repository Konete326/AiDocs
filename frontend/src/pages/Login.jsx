    ...
    <AuthLayout title="Welcome back" subtitle="Sign in to continue building">
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="neumorphic-input-wrapper rounded-xl px-4 py-2.5 flex items-center gap-3">
          ...
          <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} required className="bg-transparent text-[#3D4852] placeholder-[#9CA3AF] outline-none w-full text-sm font-medium" />
        ...
        <div className="flex items-center justify-between text-xs pt-1">
          <label className="flex items-center gap-2 cursor-pointer text-[#6B7280] hover:text-[#3D4852] select-none font-medium">
          ...
          <Link to="/forgot-password" className="text-[#6C63FF] hover:text-[#8B84FF] font-semibold transition-colors">Forgot password?</Link>
        </div>
